...
class STGothamRoundedMediumTextView : AppCompatTextView {
    ...
    private fun setCustomFont() {
        ...
        synchronized(STPCache.sharedGCache.getFontCache()) {
            if (!STPCache.sharedGCache.getFontCache()
                    .containsKey("GothamRounded-Medium")
            ) {
                val tf = Typeface.createFromAsset(
                    context!!.assets,
                    "fonts/GothamRounded-Medium.otf"
                )
                STPCache.sharedGCache.setFontCache("GothamRounded-Medium", tf)
            }
            setTypeface(
                STPCache.sharedGCache.getFontCache()["GothamRounded-Medium"],
                Typeface.NORMAL
            )
