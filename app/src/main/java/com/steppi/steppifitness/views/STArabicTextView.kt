...
class STArabicTextView : AppCompatTextView {
    ...
    private fun setCustomFont() {
        ...
        synchronized(STPCache.sharedGCache.getFontCache()) {
            if (!STPCache.sharedGCache.getFontCache()
                    .containsKey("NotoKufiArabic-Regular")
            ) {
                val tf = Typeface.createFromAsset(
                    context!!.assets,
                    "fonts/NotoKufiArabic-Regular.ttf"
                )
                STPCache.sharedGCache.setFontCache("NotoKufiArabic-Regular", tf)
            }
            setTypeface(
                STPCache.sharedGCache.getFontCache()["NotoKufiArabic-Regular"],
                Typeface.NORMAL
            )
